...
if __name__ == '__main__':
    ...
    print("trainset reshaped")
    test_images = data_obj_array_test.reshape(data_obj_array_test.shape[0], -1)

    print("testset reshaped")
    pca = PCA(n_components=100)
    train_images_pca = pca.fit_transform(train_images)
    test_images_pca = pca.transform(test_images)

    clf = SVC(kernel='linear', C=1, random_state=42, verbose=True)
    print("SVC done")
    clf.fit(train_images_pca, data_label_array_train)
    print("Fit done")
    ...
    y_true = label_binarize(data_label_array_test, classes=[0, 1, 2])
