...
class OCTDataset(Dataset):
    def __init__(self, args, subset='train', transform=None,):
        if subset == 'train':
            ...
            self._35 = self.annot[self.annot['DRSS']==35]
            self._43 = self.annot[self.annot['DRSS']==43]
            self._zero = pd.concat([self._35, self._43]).sample(n=4000).drop_duplicates()
            
            self._47 = self.annot[self.annot['DRSS']==47]
            self._53 = self.annot[self.annot['DRSS']==53]
            self._one = pd.concat([self._47, self._53]).sample(n=4000).drop_duplicates()
            
            self._61 = self.annot[self.annot['DRSS']==61]
            self._65 = self.annot[self.annot['DRSS']==65]
            self._two_1 = pd.concat([self._61, self._65])
            
            self._71 = self.annot[self.annot['DRSS']==71]
            self._85 = self.annot[self.annot['DRSS']==85]
            self._two_2 = pd.concat([self._71, self._85])
            self._two = pd.concat([self._two_1, self._two_2]).sample(n=4000).drop_duplicates()
            self._annot_0 = pd.concat([self._zero, self._one])
            self._annot = pd.concat([self._annot_0, self._two])
