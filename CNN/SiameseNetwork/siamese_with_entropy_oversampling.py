...
class OCTDataset(Dataset):
    def __init__(self, args, subset='train', transform=None,):
        if subset == 'train':
            ...
            selected_indices = np.argsort(row_entropies)[::-1][:3972]
            ...
            row_entropies = np.apply_along_axis(entropy, axis=1, arr=metadata)
            # Sort the samples in decreasing order of entropy and select the top ones
            selected_indices = np.argsort(row_entropies)[::-1][:1000]
            ...
            row_entropies = np.apply_along_axis(entropy, axis=1, arr=metadata)
            # Sort the samples in decreasing order of entropy and select the top ones
            selected_indices = np.argsort(row_entropies)[::-1][:3528]
            
            self._two = self._two.iloc[selected_indices]
            self._annot_0 = pd.concat([self.annot_x, self._one])
            self.annot_1 = pd.concat([self._annot_0, self._two])
            self.annot = pd.concat([self.annot_1, self._two])
